---
name: jjb-api-host-config
description: 定义 JJB 接口服务配置规范。在配置 API_HOST、jjb.config.js environment、process.env.app.API_HOST 时查阅。
---
...
- **配置结构**：
  ...
  // jjb.config.js
  {
    environment: {
      ...
      production: {
        API_HOST: ''  // 必须设置为空字符串，详见应用启动与编译章节
      }
    }
  }
  ```
  ...
  - **配置示例**：
    ...
    // jjb.config.js
    {
      environment: {
        ...
        production: {
          API_HOST: ''  // 生产环境必须设置为空字符串，详见应用启动与编译章节
        }
      }
    }
    ```
